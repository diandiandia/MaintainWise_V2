...
* **安全鉴权微核心**：
  ...
  - **180天生命周期守护器与超期自冻结切面**：
    ...
    - 若 $177 \le \Delta D \le 180$ 天（临期 3 天），系统在登录响应中注入 `password_expiring_soon: true` 及剩余天数，前端激活黄色预警条；
    - 若 `must_change_password` 为 true（初始密码或管理员重置密码后首次登录），前端激活不可关闭的全局强制改密模态框。
...
* **对应客户需求**：`CR-DEV-001` ~ `CR-DEV-012`。
* **数据实体模型**：
  采用 `equipments` (设备主表) 与 `equipment_runtime_logs` (运行工时抄表流水表)。
...
5. **间歇与持续双模维护预测与 14 天滑动加权算法**：
   ...
   - **14 天滑动加权日均工时智能预测**：
     系统聚合设备过去 14 天内的抄表增量 $\Delta H_i$，采用近重远轻的线性加权权重 $w_i = i$，推算近期日均开机工时：
     $$T_{\text{avg}} = \frac{\sum_{i=1}^{14} w_i \cdot \Delta H_i}{\sum_{i=1}^{14} w_i}$$
     $$\text{Estimated Days Left} = \left\lceil \frac{\text{Remaining Hours}}{\max(T_{\text{avg}}, 0.5)} \right\rceil$$
   - **自适应预警与打卡闭环**：
...
### 9.1 跨平台工程约束与技术方案裁决
* **对应客户需求**：`CR-CON-001`, `CR-CON-002`, `CR-CON-003`, `CR-CON-004`, `CR-CON-005`。
* **关键裁决与落地措施**：
  ...
  6. **双轨同构运维工具箱**：根目录与部署目录下同时提供功能完全对等的 Linux Shell 脚本与 Windows 批处理工具箱。

### 9.2 双轨部署脚本目录树
```
MaintainWise_V2/
├── 一键部署_Linux.sh               # 根目录 Linux 一键全自动综合部署入口
├── 一键部署_Windows.bat              # 根目录 Windows 一键全自动综合部署入口
│
├── deploy/                          # 统一双轨部署工具总目录
│   ├── linux/                       # Linux 专用一键运维脚本工具箱
│   │   ├── 0_一键完整部署(Linux).sh # 全自动环境校验、依赖安装与服务配置向导
│   │   ├── 1_一键环境初始化.sh      # 虚拟环境创建与 SQLite 数据表种子注入
│   │   ├── 2_前台测试启动.sh        # 单端口 8000 前台交互测试启动
│   │   ├── 3_安装Systemd服务.sh     # 自动生成并注册 systemd 系统守护服务
│   │   ├── 4_启动服务.sh            # 启动 Linux 后台服务
│   │   ├── 5_停止服务.sh            # 停止 Linux 后台服务
│   │   ├── 6_卸载Systemd服务.sh     # 卸载 systemd 服务 (数据完好保留)
│   │   ├── 7_立即执行备份.sh        # 一键打包 SQLite+附件为带时间戳 ZIP
