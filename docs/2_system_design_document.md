...
5. **工厂/部门/系统层级更名与设备/层级安全软删除**：
   ...
   - 支持层级级联软删除与单设备软删除：
     - **层级软删除**：当用户确认关停某层级时，系统自动级联软删除从属的所有设备；
     - **单设备删除与四级树**：左侧架构树延伸至第四级（工厂 -> 车间 -> 系统 -> 设备），系统节点下直观展示归属设备列表；在设备节点、表格列表与看板大卡片上均提供【🗑️ 移除设备】按钮；删除单台设备后，所属系统架构依然保存在架构表中（计数更新为0），历史工单与维保档案 100% 永久留存。
...
5. **维修复盘根本原因、排除步骤与完工修复照片强制闭环（核心传承资产）**：
   ...
   - **完工修复照片双通道采集 (`repair_photos`)**：
     - 支持现场移动端/手机/平板**直接拍照上传**（调用后置摄像头实时拍摄修复后设备运转、仪表及接线达标照片）；
     - 支持**从本地图库/电脑相册导入**（支持多选批量导入高清照片）；
     - 上传后即时生成可交互缩略图列表，支持单张移除与点击全屏放大预览，作为试车核验与后来人终身病历的法定佐证；
   - 未填报核心复盘内容时，系统接口级拒绝提交完工。
